...
export function useTypingEngine({ text, active, startedAt = null, endsAt = null, errorRunway = 8, onFinish, onProgress, }) {
    ...
    const correctChars = useMemo(() => {
        let i = 0;
        while (i < typed.length && i < text.length && typed[i] === text[i])
            i++;
        return i;
    }, [typed, text]);
    const charStates = useMemo(() => {
        ...
        for (let i = 0; i < text.length; i++) {
            if (i < typed.length)
                states[i] = typed[i] === text[i] ? "correct" : "wrong";
    ...
    const snapshot = useMemo(() => {
        ...
        const accuracy = keystrokesRef.current === 0 ? 100 : Math.min(100, (correctChars / denominator) * 100);
        return {
            correctChars,
     ...
     */
    const applyValue = useCallback((raw) => {
        ...
        if (next.length > current.length) {
            ...
            let correct = 0;
            while (correct < next.length && next[correct] === text[correct])
                correct++;
    ...
    const handleChange = useCallback((event) => applyValue(event.target.value), [applyValue]);
    /** Type one character, as a tap on the on screen keyboard does. */
    const typeChar = useCallback((char) => applyValue(typedRef.current + char), [applyValue]);
    ...
    const handleCopyCut = useCallback((event) => {
        event.preventDefault();
    }, []);
    /**
     * Take focus back.
    ...
    const handleBlur = useCallback(() => {
        window.setTimeout(() => {
            ...
            const movedToAnotherField = el instanceof HTMLElement &&
                (el.tagName === "INPUT" ||
                    el.tagName === "TEXTAREA" ||
                    el.tagName === "SELECT" ||
                    el.isContentEditable);
            if (movedToAnotherField)
                return;
    ...
    useEffect(() => {
        ...
        const onKeyDown = (event) => {
            ...
            const inAnotherField = el instanceof HTMLElement &&
                (el.tagName === "INPUT" ||
                    el.tagName === "TEXTAREA" ||
                    el.tagName === "SELECT" ||
                    el.isContentEditable);
