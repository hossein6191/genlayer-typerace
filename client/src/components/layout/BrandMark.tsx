 ...
 */
export function StrongMark({ className, glow }: { className?: string; glow?: boolean }) {
  return (
    <svg
      ...
      role="img"
      aria-label="GenLayer"
      style={glow ? { filter: "drop-shadow(0 0 10px var(--color-gl-purple))" } : undefined}
    >
      <polygon
...
export function BrandLockup({ className }: { className?: string }) {
  ...
  return (
    <span className={cn("flex flex-col leading-none", className)}>
      <img
        src="/brand/svg/GenLayer_Logo_White_Cropped.svg"
        alt="GenLayer"
        className="h-[17px] w-auto"
        width={385}
        height={92}
      />
      <span className="mt-1.5 pl-[2.4rem] text-[9px] font-bold uppercase tracking-[0.34em] text-gl-purple">
        TypeRace
      </span>
    </span>
  );
