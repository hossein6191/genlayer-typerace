...
function getSoundCategory(id) {
    if (id === "space")
        return "spacebar";
    if (MODIFIER_KEY_IDS.has(id))
        return "modifier";
    return "normal";
}
const MODIFIER_DISPLAY_ORDER = ["Ctrl", "Shift", "Alt", "Win", "Fn"];
function getKeyDisplayLabel(id) {
    if (id === "space")
        return "Space";
    return ALL_KEYS_BY_ID[id]?.label || id;
}
function getActiveKeyParts(ids) {
    if (ids.length === 0)
        return null;
...
export function keyIdForChar(char) {
...
function getKeyVariance(id, small) {
...
function getThockEngine() {
    if (thockEngine)
        return Promise.resolve(thockEngine);
    if (thockEnginePromise)
...
export function unlockKeyboardAudio() {
    if (typeof window === "undefined" || !soundEnabled)
        return;
    if (thockEngine) {
        if (thockEngine.ctx.state === "suspended")
            void thockEngine.ctx.resume();
        return;
    }
...
function playKeySound(category, muted, panHint = 0) {
    if (typeof window === "undefined" || !soundEnabled)
        return;
    void getThockEngine().then((engine) => {
        ...
        body.frequency.exponentialRampToValueAtTime(profile.bodyHz * detune * 0.72, now + bodySeconds);
...
const THEMES = {
    vintage: {
        ...
        topRingShadow: "inset 0 0 0 0.75px rgba(96,70,42,0.28), inset 0 0.6px 0 rgba(255,250,238,0.4), inset 0 -0.8px 1.2px rgba(15,9,4,0.04)",
        topRingShadowPressed: "inset 0 0 0 0.75px rgba(96,70,42,0.34), inset 0 0.5px 0 rgba(255,250,238,0.22), inset 0 1px 2px rgba(15,10,5,0.1)",
    ...
    `,
        caseShadow: "0 0.5px 0 rgba(255,222,185,0.18) inset, 0 -2px 4.5px rgba(35,19,6,0.32) inset, 0.4px 0.4px 0.8px rgba(255,232,200,0.14) inset, 0 3px 6px rgba(15,8,3,0.22), 0 1px 2px rgba(15,8,3,0.2)",
        bezelBackground: "linear-gradient(155deg, #15120e 0%, #0e0c08 50%, #0a0805 100%)",
        bezelShadow: "inset 0 2.5px 6px rgba(0,0,0,0.55), inset 0 4px 8px rgba(0,0,0,0.28), inset 0 -1px 0 rgba(255,255,255,0.04), inset 0 0.5px 0 rgba(255,255,255,0.05), inset 0 0 0 1px rgba(0,0,0,0.32), 0 1px 0 rgba(255,236,204,0.1)",
        bezelSheen: "radial-gradient(140% 60% at 44% 0%, rgba(180,120,70,0.1), transparent 45%)",
        caseEdge: "inset 0 1px 0 rgba(255,246,224,0.6), inset 0 -1px 0 rgba(10,6,2,0.55), inset 1px 0 0 rgba(255,246,224,0.26), inset -1px 0 0 rgba(10,6,2,0.36)",
        woodTextures: true,
    ...
    genlayer: {
        ...
        wallShadow: "inset 0 1px 0 rgba(180,190,255,0.22), inset 0.6px 0.4px 0 rgba(180,190,255,0.08), inset 0 -1.5px 2px rgba(0,0,0,0.45), inset 0 0 0 0.5px rgba(0,0,0,0.35)",
        topRingShadow: "inset 0 0 0 0.75px rgba(120,130,220,0.28), inset 0 0.6px 0 rgba(200,205,255,0.18), inset 0 -0.8px 1.2px rgba(0,0,0,0.35)",
        topRingShadowPressed: "inset 0 0 0 0.75px rgba(155,106,246,0.55), inset 0 0.5px 0 rgba(200,205,255,0.1), inset 0 1px 2px rgba(0,0,0,0.5)",
    ...
    `,
        caseShadow: "0 0.5px 0 rgba(200,205,255,0.16) inset, 0 -2px 4.5px rgba(0,0,0,0.5) inset, 0 10px 34px -14px rgba(155,106,246,0.5), 0 3px 6px rgba(0,0,0,0.5)",
        bezelBackground: "linear-gradient(155deg, #0B0D1C 0%, #070914 50%, #04050D 100%)",
        bezelShadow: "inset 0 2.5px 6px rgba(0,0,0,0.7), inset 0 4px 8px rgba(0,0,0,0.4), inset 0 -1px 0 rgba(200,205,255,0.05), inset 0 0 0 1px rgba(0,0,0,0.5), 0 1px 0 rgba(155,106,246,0.18)",
        bezelSheen: "radial-gradient(140% 60% at 44% 0%, rgba(155,106,246,0.16), transparent 48%)",
        caseEdge: "inset 0 1px 0 rgba(205,210,255,0.28), inset 0 -1px 0 rgba(0,0,0,0.65), inset 1px 0 0 rgba(205,210,255,0.12), inset -1px 0 0 rgba(0,0,0,0.5)",
        woodTextures: false,
...
function resolveTier() {
    ...
    if (window.matchMedia(MOBILE_BREAKPOINT).matches)
        return "mobile";
    if (window.matchMedia(TABLET_BREAKPOINT).matches)
        return "tablet";
    return "desktop";
}
...
const Key = memo(function Key({ config, rowIndex, tier, theme, compact, isNext, registerTrigger, onActivate, onDeactivate, onTap, }) {
    const { id, label, shiftLabel, width = 1, align = "center", small, muted, } = config;
    ...
    const displayLabel = tier === "mobile" ? (MOBILE_LABEL_OVERRIDES[id] ?? label) : label;
    ...
    const layers = useMemo(() => {
        ...
        return {
            insetTRBL,
            wallGradient: `linear-gradient(180deg, ${shiftLightness(w0, variance.lightnessShift)} 0%, ${shiftLightness(w1, variance.lightnessShift)} 18%, ${shiftLightness(w2, variance.lightnessShift)} 46%, ${shiftLightness(w3, variance.lightnessShift * 0.7)} 78%, ${shiftLightness(w4, variance.lightnessShift * 0.5)} 100%)`,
            ...
            topGradient: `radial-gradient(115% 125% at ${23 + variance.specularShiftX * 0.4}% 9%, rgba(255,255,255,${0.4 - variance.wearAmount * 0.06}), rgba(255,255,255,0) 44%), radial-gradient(150% 120% at 50% 118%, rgba(15,9,4,${0.07 + variance.wearAmount * 0.02}), transparent 60%), ${shiftLightness(theme.keycap, variance.lightnessShift * 0.6)}`,
    ...
    return (_jsxs("button", { type: "button", "aria-label": label || "Space", tabIndex: -1, "data-pressed": pressed, "data-next": isNext || undefined, onPointerDown: handlePress, onPointerUp: handleRelease, onPointerCancel: handleRelease, onPointerLeave: handleRelease, style: {
            flexGrow: width,
            flexBasis: 0,
            minWidth: 0,
            height: keyHeight,
            "--tilt": `${variance.microTilt}deg`,
        }, className: "kb-key relative select-none outline-none", children: [_jsx("span", { className: "pointer-events-none absolute", style: {
                    inset: 0,
                    ...
                    zIndex: 0,
                } }), _jsx("span", { className: "absolute inset-0", style: {
                    borderRadius: radius.wall,
                    background: layers.wallGradient,
                    filter: layers.wallFilter,
                    boxShadow: layers.wallShadow,
                    zIndex: 1,
                } }), _jsx("span", { className: "pointer-events-none absolute inset-0 mix-blend-overlay", style: {
                    borderRadius: radius.wall,
                    ...
                    zIndex: 1,
                } }), _jsx("span", { className: "absolute", style: {
                    ...
                    zIndex: 3,
                } }), _jsx("span", { className: "pointer-events-none absolute mix-blend-overlay", style: {
                    borderRadius: radius.top,
                    inset: layers.insetTRBL,
                    backgroundImage: `url("${PBT_NOISE_URI}")`,
                    ...
                    zIndex: 3,
                } }), _jsx("span", { className: "pointer-events-none absolute", style: {
                    borderRadius: radius.top,
                    inset: layers.insetTRBL,
                    ...
                    transition: "opacity 140ms ease-out",
                    zIndex: 4,
                } }), isNext && (_jsx("span", { className: "kb-next-ring pointer-events-none absolute", style: {
                    borderRadius: radius.top,
                    inset: layers.insetTRBL,
                    ...
                    zIndex: 4,
                } })), _jsxs("span", { className: "pointer-events-none absolute inset-0", style: { zIndex: 5 }, children: [shiftLabel && (_jsx("span", { className: "absolute font-medium leading-none", style: {
                        ...
                        }, children: shiftLabel })), label && (_jsx("span", { className: `absolute leading-none ${small ? "font-semibold" : "font-bold"} ${primaryAlign === "left" ? "text-left" : "text-center"}`, style: {
                            bottom: `calc(${sculpt.insetBottom}px + ${LEGEND_SHARED.primaryBottomOffset})`,
                            left: primaryAlign === "left"
                                ? LEGEND_SHARED.primaryLeftOffset
                                : shiftLabel
                                    ? `calc(50% - ${LEGEND_SHARED.opticalCenterShift})`
                                    : "50%",
                            transform: primaryAlign === "left" ? undefined : "translateX(-50%)",
...
export const VintageKeyboard = ({ variant = "vintage", embedded = false, compact = false, showIndicator, sound = true, highlightChar = null, listenWhileTyping = false, onType, onBackspace, className, maxWidth, }) => {
    const rows = useMemo(() => ROWS, []);
    const keyTriggersRef = useRef({});
    ...
    const setShift = useCallback((next) => {
    ...
    const handleTap = useCallback((id) => {
        ...
        if (shiftLatchedRef.current)
            setShift(false);
    }, [onType, onBackspace, setShift]);
    const registerTrigger = useCallback((id, trigger) => {
    ...
    const [indicatorParts, setIndicatorParts] = useState(null);
    const [indicatorVisible, setIndicatorVisible] = useState(true);
    const indicatorPartsRef = useRef(null);
    const holdTimeoutRef = useRef(null);
    const fadeTimeoutRef = useRef(null);
    useEffect(() => {
        indicatorPartsRef.current = indicatorParts;
    }, [indicatorParts]);
    const activateKey = useCallback((id) => {
        setActiveKeyIds((prev) => (prev.includes(id) ? prev : [...prev, id]));
    }, []);
    const deactivateKey = useCallback((id) => {
    ...
    useEffect(() => {
        ...
        const handleKeyDown = (event) => {
            ...
            if (event.repeat)
                return;
            if (!listenWhileTyping && isTypingTarget(event.target))
                return;
    ...
    return (_jsxs("div", { className: [
            ...
            background: embedded ? "transparent" : theme.pageBackground,
        }, "aria-hidden": embedded || undefined, children: [_jsx("style", { children: KEY_STYLE_TAG }), _jsxs("div", { className: "flex flex-col items-center", style: { width: "100%", maxWidth: maxWidth ?? container.maxWidth }, children: [withIndicator && (_jsx("div", { className: "flex items-center justify-center", style: {
                            ...
                            fontFamily: "var(--font-sans, Inter, system-ui, sans-serif)",
                                ...
                                transition: "opacity 220ms ease-out",
                            }, children: indicatorParts && indicatorParts.length > 0 ? (_jsx("div", { className: "flex items-center justify-center", style: { gap: "5px" }, children: indicatorParts.map((part, i) => (_jsxs(Fragment, { children: [i > 0 && (_jsx("span", { style: {
                                                fontSize: "0.72rem",
                                                fontWeight: 500,
                                                ...
                                                borderRadius: "6px",
                                                padding: "5px 8px",
                                                boxShadow: "0 1px 0 rgba(255,255,255,0.7) inset, 0 1px 2px rgba(0,0,0,0.04), 0 1px 1px rgba(0,0,0,0.03)",
                                                letterSpacing: "0.01em",
                                            }, children: part })] }, `${part}-${i}`))) })) : (_jsx("span", { style: {
                                    fontSize: "clamp(0.8rem, 1.5vw, 0.9rem)",
                                    fontWeight: 500,
                                    letterSpacing: "0.01em",
                                    color: theme.indicatorText,
                                }, children: "Press any key..." })) }) })), _jsx("div", { style: {
                            perspective: "1800px",
                            width: "100%",
                        }, children: _jsxs("div", { className: "relative w-full", style: { transform: "rotateX(7deg)", transformOrigin: "50% 100%" }, children: [_jsx("div", { className: "absolute inset-x-[16%] top-[99%] -z-10 h-1 rounded-full blur-[1.5px]", style: { background: "rgba(15,10,6,0.2)" } }), _jsx("div", { className: "absolute -inset-x-2 top-14 bottom-0 -z-10 rounded-[1.5rem] blur-lg", style: {
                                        background: "radial-gradient(55% 70% at 50% 82%, rgba(15,10,6,0.06), transparent 72%)",
                                    } }), _jsxs("div", { className: "relative rounded-[var(--kb-case-radius)]", style: {
                                        ...
                                        "--kb-bezel-radius": caseTier.bezelRadius,
                                    }, children: [theme.woodTextures && (_jsxs(_Fragment, { children: [_jsx("div", { className: "pointer-events-none absolute inset-0 rounded-[var(--kb-case-radius)] mix-blend-multiply", style: {
                                                        backgroundImage: `url("${WOOD_TONE_URI}")`,
                                                        backgroundSize: "520px 520px",
                                                        opacity: 0.46,
                                                    } }), _jsx("div", { className: "pointer-events-none absolute inset-0 rounded-[var(--kb-case-radius)] mix-blend-multiply", style: {
                                                        ...
                                                        opacity: 0.24,
                                                    } }), _jsx("div", { className: "pointer-events-none absolute inset-0 rounded-(--kb-case-radius) mix-blend-overlay", style: {
                                                        background: "repeating-linear-gradient(179deg, rgba(255,228,192,0.065) 0px, transparent 3px, transparent 17px, rgba(45,23,7,0.1) 20px, transparent 29px), repeating-linear-gradient(183deg, rgba(255,228,192,0.032) 0px, transparent 7px, transparent 41px, rgba(45,23,7,0.055) 44px, transparent 59px)",
                                                        opacity: 0.58,
                                                    } }), _jsx("div", { className: "pointer-events-none absolute inset-0 rounded-(--kb-case-radius) mix-blend-multiply", style: {
                                                        backgroundImage: `url("${WOOD_PORE_URI}")`,
                                                        backgroundSize: "130px 130px",
                                                        opacity: 0.34,
                                                    } }), _jsx("div", { className: "pointer-events-none absolute inset-0 rounded-(--kb-case-radius) mix-blend-screen", style: {
                                                        backgroundImage: `url("${WOOD_MICROSCRATCH_URI}")`,
                                                        backgroundSize: "620px 420px",
                                                        opacity: 0.5,
                                                    } }), _jsx("div", { className: "pointer-events-none absolute inset-0 rounded-(--kb-case-radius)", style: {
                                                        backgroundImage: `url("${WOOD_DENT_URI}")`,
                                                        backgroundSize: "100% 100%",
                                                        opacity: 0.28,
                                                    } }), _jsx("div", { className: "pointer-events-none absolute rounded-tl-(--kb-case-radius) mix-blend-multiply", style: {
                                                        left: 0,
                                                        top: 0,
                                                        width: "9%",
                                                        height: "18%",
                                                    ...
                                                    } }), _jsx("div", { className: "pointer-events-none absolute rounded-br-(--kb-case-radius) mix-blend-multiply", style: {
                                                        right: 0,
                                                        bottom: 0,
                                                        width: "10%",
                                                        height: "20%",
                                                        ...
                                                        backgroundSize: "80px 80px",
                                                        opacity: 0.5,
                                                        maskImage: "radial-gradient(ellipse at bottom right, black, transparent 75%)",
                                                    } })] })), _jsx("div", { className: "pointer-events-none absolute inset-0 rounded-(--kb-case-radius)", style: {
                                                background: "linear-gradient(112deg, transparent 30%, rgba(255,244,222,0.06) 44%, rgba(255,244,222,0.1) 49%, rgba(255,244,222,0.05) 54%, transparent 68%)",
                                                mixBlendMode: "screen",
                                            } }), _jsx("div", { className: "pointer-events-none absolute inset-0 rounded-(--kb-case-radius)", style: {
                                                background: "radial-gradient(85% 50% at 38% -8%, rgba(255,240,210,0.15), transparent 42%)",
                                            } }), _jsx("div", { className: "pointer-events-none absolute inset-x-0 bottom-0 rounded-b-[var(--kb-case-radius)]", style: {
                                                height: "14%",
                                                background: "linear-gradient(180deg, transparent 0%, rgba(250,248,244,0.05) 100%)",
                                                mixBlendMode: "screen",
                                            } }), _jsx("div", { className: "pointer-events-none absolute inset-0 rounded-[var(--kb-case-radius)]", style: { boxShadow: theme.caseEdge } }), _jsxs("div", { className: "relative rounded-[var(--kb-bezel-radius)]", style: {
                                                padding: caseTier.bezelPadding,
                                                background: theme.bezelBackground,
                                                boxShadow: theme.bezelShadow,
                                            }, children: [_jsx("div", { className: "pointer-events-none absolute inset-0 rounded-[var(--kb-bezel-radius)]", style: { background: theme.bezelSheen, zIndex: 0 } }), _jsx("div", { className: "relative z-10 flex flex-col", style: { gap }, children: rows.map((row, i) => (_jsx("div", { className: "flex", style: { gap }, children: row.map((key) => (_jsx(Key, { config: key, rowIndex: i, tier: tier, theme: theme, compact: compact, isNext: nextKeys.has(key.id) ||
                                                                (shiftLatched && (key.id === "lshift" || key.id === "rshift")), registerTrigger: registerTrigger, onActivate: activateKey, onDeactivate: deactivateKey, onTap: handleTap }, key.id))) }, i))) })] })] })] }) })] })] }));
};
